...
mutable struct C2S
    ...
    refreshCache::Bool
    C2S(; header = Trd_Common.TrdHeader(), filterConditions = Trd_Common.TrdFilterConditions(), filterStatusList = Vector{Int32}(), refreshCache = false) = new(header, filterConditions, filterStatusList, refreshCache)
end

PB.default_values(::Type{C2S}) = (;header = Trd_Common.TrdHeader(), filterConditions = Trd_Common.TrdFilterConditions(), filterStatusList = Vector{Int32}(), refreshCache = false)
PB.field_numbers(::Type{C2S}) = (;header = 1, filterConditions = 2, filterStatusList = 3, refreshCache = 4)

function PB.encode(e::PB.AbstractProtoEncoder, x::C2S)
    initpos = position(e.io)
    PB.encode(e, 1, x.header)
    PB.encode(e, 2, x.filterConditions)
    !isempty(x.filterStatusList) && PB.encode(e, 3, x.filterStatusList)
    x.refreshCache != false && PB.encode(e, 4, x.refreshCache)
