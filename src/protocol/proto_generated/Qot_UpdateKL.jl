module Qot_UpdateKL

import ProtoBuf as PB
import ..Common
import ..Qot_Common

mutable struct S2C
    rehabType::Int32 #Qot_Common.RehabType,复权类型
    klType::Int32 #Qot_Common.KLType,K线类型
    security::Qot_Common.Security #股票
    name::String # 股票名称
    klList::Vector{Qot_Common.KLine} #推送的k线点
    S2C(; rehabType = Int32(0), klType = Int32(0), security = Qot_Common.Security(), name = "", klList = Vector{Qot_Common.KLine}()) = new(rehabType, klType, security, name, klList)
end

PB.default_values(::Type{S2C}) = (; rehabType = Int32(0), klType = Int32(0), security = Qot_Common.Security(), name = "", klList = Vector{Qot_Common.KLine}())
PB.field_numbers(::Type{S2C}) = (; rehabType = 1, klType = 2, security = 3, klList = 4, name = 5)
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:S2C})
    rehabType = Int32(0)
    klType = Int32(0)
    security = Qot_Common.Security()
    name = ""
    klList = Vector{Qot_Common.KLine}()
    while !PB.message_done(d)
        field_number, wire_type = PB.decode_tag(d)
        if field_number == 1
            rehabType = PB.decode(d, Int32)
        elseif field_number == 2
            klType = PB.decode(d, Int32)
        elseif field_number == 3
            security = PB.decode(d, Ref{Qot_Common.Security})
        elseif field_number == 4
            push!(klList, PB.decode(d, Ref{Qot_Common.KLine}))
        elseif field_number == 5
            name = PB.decode(d, String)
        else
            PB.skip(d, wire_type)
        end
    end
    return S2C(; rehabType = rehabType, klType = klType, security = security, name = name, klList = klList)
end

mutable struct Response
    retType::Int32    # RetType,返回结果
    retMsg::String
    errCode::Int32
    s2c::S2C
    Response(; retType = Int32(-400), retMsg = "", errCode = Int32(0), s2c = S2C()) = new(retType, retMsg, errCode, s2c)
end

PB.default_values(::Type{Response}) = (; retType = Int32(-400), retMsg = "", errCode = Int32(0), s2c = S2C())
PB.field_numbers(::Type{Response}) = (; retType = 1, retMsg = 2, errCode = 3, s2c = 4)
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:Response})
    retType = Int32(-400)
    retMsg = ""
    errCode = Int32(0)
    s2c = S2C()
    while !PB.message_done(d)
        field_number, wire_type = PB.decode_tag(d)
        if field_number == 1
            retType = PB.decode(d, Int32)
        elseif field_number == 2
            retMsg = PB.decode(d, String)
        elseif field_number == 3
            errCode = PB.decode(d, Int32)
        elseif field_number == 4
            s2c = PB.decode(d, Ref{S2C})
        else
            PB.skip(d, wire_type)
        end
    end
    return Response(; retType = retType, retMsg = retMsg, errCode = errCode, s2c = s2c)
end

export S2C, Response

end
