...
mutable struct C2S
    ...
    refreshCache::Bool
    C2S(; header = Trd_Common.TrdHeader(), filterConditions = Trd_Common.TrdFilterConditions(), refreshCache = false) = new(header, filterConditions, refreshCache)
end

PB.default_values(::Type{C2S}) = (;header = Trd_Common.TrdHeader(), filterConditions = Trd_Common.TrdFilterConditions(), refreshCache = false)
PB.field_numbers(::Type{C2S}) = (;header = 1, filterConditions = 2, refreshCache = 3)

function PB.encode(e::PB.AbstractProtoEncoder, x::C2S)
    initpos = position(e.io)
    PB.encode(e, 1, x.header)
    PB.encode(e, 2, x.filterConditions)
    x.refreshCache != false && PB.encode(e, 3, x.refreshCache)
