...
mutable struct S2C
    ...
    name::String #股票名称
    tickerList::Vector{Qot_Common.Ticker} #逐笔
    S2C(; security = Qot_Common.Security(), name = "", tickerList = Vector{Qot_Common.Ticker}()) = new(security, name, tickerList)
end
...
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:S2C})
    ...
    return S2C(; security = security, name = name, tickerList = tickerList)
end
