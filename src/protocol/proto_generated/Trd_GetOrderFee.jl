...
mutable struct C2S
    ...
    orderIdExList::Vector{String}
    C2S(; header = Trd_Common.TrdHeader(), orderIdExList = Vector{String}()) = new(header, orderIdExList)
end

PB.default_values(::Type{C2S}) = (;header = Trd_Common.TrdHeader(), orderIdExList = Vector{String}())
PB.field_numbers(::Type{C2S}) = (;header = 1, orderIdExList = 2)

function PB.encode(e::PB.AbstractProtoEncoder, x::C2S)
    initpos = position(e.io)
    PB.encode(e, 1, x.header)
    foreach(item -> PB.encode(e, 2, item), x.orderIdExList)
    return position(e.io) - initpos
end

mutable struct S2C
    header::Trd_Common.TrdHeader
    orderFeeList::Vector{Trd_Common.OrderFee}
    S2C(; header = Trd_Common.TrdHeader(), orderFeeList = Vector{Trd_Common.OrderFee}()) = new(header, orderFeeList)
end

PB.default_values(::Type{S2C}) = (;header = Trd_Common.TrdHeader(), orderFeeList = Vector{Trd_Common.OrderFee}())
PB.field_numbers(::Type{S2C}) = (;header = 1, orderFeeList = 2)
...
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:S2C})
    ...
    while !PB.message_done(d)
        ...
        elseif field_number == 2
            push!(orderFeeList, PB.decode(d, Ref{Trd_Common.OrderFee}))
        else
            PB.skip(d, wire_type)
        end
    end
    return S2C(header = header, orderFeeList = orderFeeList)
end
